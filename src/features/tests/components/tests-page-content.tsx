...
export function TestsPageContent({ initialTestId }: { initialTestId?: string } = {}) {
  ...
  const filtered = useMemo(() => filterTestsByTitle(items, titleQuery), [items, titleQuery]);
...
function QuestionCountHint({ testId }: { testId: string }) {
  ...
  return <p className="text-xs text-muted-foreground">عدد الأسئلة: {items.length}</p>;
}
