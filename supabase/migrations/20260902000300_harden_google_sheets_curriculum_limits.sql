...
begin
  ...
  if length(trim(p_grade)) > 200 then
    raise exception 'GRADE_TOO_LONG';
  end if;

  if nullif(trim(coalesce(p_semester, '')), '') is null then
    raise exception 'MISSING_SEMESTER';
  end if;
  if length(trim(p_semester)) > 200 then
    raise exception 'SEMESTER_TOO_LONG';
  end if;

  if p_academic_year is not null and length(trim(p_academic_year)) > 200 then
    raise exception 'INVALID_ACADEMIC_YEAR';
  end if;

  if p_original_name is not null and length(trim(p_original_name)) > 200 then
    raise exception 'INVALID_ORIGINAL_NAME';
  end if;
  ...
  loop
    ...
    if length(v_title) > 500 then
      raise exception 'LESSON_TITLE_TOO_LONG';
    end if;

    if length(coalesce(v_lesson->>'objectives', '')) > 10000 then
      raise exception 'OBJECTIVES_TOO_LONG';
    end if;

    if length(coalesce(v_lesson->>'notes', '')) > 10000 then
      raise exception 'NOTES_TOO_LONG';
    end if;

    if length(coalesce(v_lesson->>'unitTitle', '')) > 100 then
      raise exception 'UNIT_TITLE_TOO_LONG';
    end if;
